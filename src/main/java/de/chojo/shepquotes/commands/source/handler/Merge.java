...
public class Merge implements SlashHandler {
    private final QuoteData quoteData;
    private final Source source;

    public Merge(QuoteData quoteData, Source source) {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event, EventContext context) {
        ...
        if (optSource.isEmpty()) {
            event.reply(context.localize("error.noSource", Replacement.create("NAME", mergeName)
        ...
        if (optTarget.isEmpty()) {
            event.reply(context.localize("error.noSource", Replacement.create("NAME", intoName)
                         .addFormatting(Format.CODE)))
                 .setEphemeral(true).queue();
            return;
        }
